    ...
    <>
      {open&&
        <div
          ...
          style={{position: 'fixed', zIndex:'99', width:'100%', height:'100%', left:0, top:0, backgroundColor:'rgba(0,0,0,0.4)'}}/>
      }
      <Container>
              ...
              <List>
                {['프로필'].map((text, index) => (
                  <ListItem key={text} disablePadding>
                    <ListItemButton onClick={handleDrawerClose} component={Link} to={index === 0? `/key-we-board-page/profile/${user.uid}`: `/key-we-board-page/info/${user.uid}`}>
                      <ListItemIcon>
          ...
          <Divider />
          {
            ...
            <List>
              {['내 글', '북마크'].map((text, index) => (
                <ListItem key={text} disablePadding>
                  <ListItemButton onClick={handleDrawerClose} component={Link} to={index === 0? `/key-we-board-page/writing/${user.uid}`: `/key-we-board-page/bookmark/${user.uid}`}>
                    <ListItemIcon>
